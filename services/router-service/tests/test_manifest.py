...
from router_service.context import Criterion, RouterContext, TaskFacts, render
...
def test_the_manifest_is_not_read_from_the_workspace_root(data_dir: Path) -> None:
    write_manifest(data_dir, manifest_document())

    assert manifest_path().parent == settings().workspace_root.parent
...
def test_nothing_outside_the_named_capabilities_is_carried(data_dir: Path) -> None:
    write_manifest(data_dir, manifest_document())
    facts = to_machine_facts(load_manifest())
    names = [item.name for item in facts.capabilities]
...
def test_a_loaded_manifest_reaches_the_rendered_prompt(data_dir: Path) -> None:
    write_manifest(data_dir, manifest_document())
    context = RouterContext(
        ...
        criteria=[
            Criterion(
                criterion_id="c1",
                statement="The database file exists",
                check_kind="deterministic",
                status="unverified",
            )
        ],
        scope_root=r"C:\Users\dev\bison\workspace",
        machine=to_machine_facts(load_manifest()),
    )
    rendered = render(context)

    assert "- os: Windows 11 26100" in rendered
    assert "- sandbox: job_object (medium)" in rendered
    assert "- ocr: none (unavailable)" in rendered
